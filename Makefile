...
run_docker=docker run -v $(shell pwd):/app -w /app --rm -it guzzle-wsse-middleware

.PHONY: help install update sh check check-native test test-native

help: ## This help message
	@fgrep -h "##" $(MAKEFILE_LIST) | fgrep -v fgrep | sed -e 's/\\$$//' -e 's/:.*#/: #/' | column -t -s '##'

build-docker-image:
	docker build -t guzzle-wsse-middleware .
